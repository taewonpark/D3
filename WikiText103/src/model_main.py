...
from WikiText103.src.utils.D3 import LinearD3Layer
...
class SlotHeadAttn(nn.Module):
    def __init__(self, n_head, d_model, d_head, dropout, dropatt=0, 
                 pre_lnorm=False):
        super(SlotHeadAttn, self).__init__()
...
class MemTransformerLM(nn.Module):
    def __init__(self,
        ...
        self.n_decomposition_layer = n_decomposition_layer
        ...
        elif attn_type in [1004]:
            for i in range(n_layer):
                if i % self.n_decomposition_layer == 0:
                    self.layers.append(
                        DecoderLayer(
                            n_head, d_model, d_head, d_inner, dropout,
                            dropatt=dropatt, pre_lnorm=pre_lnorm,
                            ...
                            attn_type=attn_type, layer_id=i, num_layer=n_layer,
                            skip_attn_normalization=skip_attn_normalization,
                            slot_size=slot_size, n_keys=n_keys, module_type=module_type,
                        )
                    )
                else:
                    self.layers.append(
                        DecoderLayer(
                            ...
                            use_out_proj=(not remove_out_proj),
                            d_res=self.d_res,
                            attn_type=34, layer_id=i, num_layer=n_layer,
                            skip_attn_normalization=skip_attn_normalization)
                    )
    ...
    def forward(self, data, target, *mems, softmax_keep_order=False,
        ...
        # Moreover, have to return new_mems to allow nn.DataParallel to pieceMemTransformerLM
        # them together.
        if not mems:
            mems = self.init_mems()
